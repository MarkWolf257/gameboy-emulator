...
static inline void
add_a_r8(register8_t r8, const char *name, bool carry)
{
...
static inline void
sub_a_r8(register8_t r8, const char *name, bool carry)
{
  hf = (af.reg.hi & 0xf) < (r8 & 0xf);
  cf = af.reg.hi < r8;
  af.reg.hi -= (r8 + (carry ? cf : 0));
...
static inline void
and_a_r8(register8_t r8, const char *name)
{
  af.reg.hi &= r8;
  zf = !(af.reg.hi);
  nf = 0; hf = 1; cf =0;
...
static inline void
xor_a_r8(register8_t r8, const char *name)
{
  af.reg.hi ^= r8;
  zf = !(af.reg.hi);
  nf = 0; hf = 0; cf =0;
...
static inline void
or_a_r8(register8_t r8, const char *name)
{
  af.reg.hi |= r8;
  zf = !(af.reg.hi);
  nf = 0; hf = 0; cf =0;
...
static inline void
cp_a_r8(register8_t r8, const char *name)
{
  zf = af.reg.hi == r8;
  nf = 1;
  hf = (af.reg.hi & 0xf) < (r8 & 0xf);
